...
async function getBook(ISBN, published_date) {
    try {
        const input = {
...
async function listBook(category, published_date) {
    try {
        const input = {
            TableName: config.TableName,
...
async function run() {
    const book = await getBook("111111111", "2022-08-07T20:20:10.528Z");
    console.log('[I] book:', book);
    const bookList = await listBook("earth", "2022-07");
